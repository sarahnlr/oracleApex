...
echo "Creating container: $storage_domain/$storage_container"
curl -v  https://storage.us2.oraclecloud.com/v1/$storage_domain/$storage_container -X PUT -H "Content-Length: 0" -H "X-Auth-Token: ${auth_token}" -o response.txt
